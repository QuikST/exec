...
draw.SimpleText("Radar","]]..b.menufont..[[",5,2.5,hacka.titletext) Поверхность.Поверхность SetDrawColor(hacka.border).DrawOutlinedRect(0,0,t,u) end end if isValid(hacka.ac) и не гей["WH_RADAR"], не паникей и не ИсВалид(hacka.ac) затем hacka.ac:Remove() end if gay["WH_INFO"] and not isValid(hacka.info) тогда hacka.info = вгуи.Create("DFrame") hacka.info:setSize(gay["WH_INFO_S_X"], gay["WH_INFO_S_Y"]) hacka.info:SetPos(gay["WH_INFO_X"], gay["WH_INFO_Y"]) hacka.info:setTitle("") hacka.info:SetSizable(true) hacka.info:ShowCloseButton(false) hacka.info.PerformLayout = function(self, c, d) zapic("WH_INFO_S_X", c)  ​zapic("WH_INFO_S_Y", d) конечная функция hacka.info:Paint(w, h) draw.RoundedBox(0,0,0,w,h,hacka.windowbg) draw.RoundedBox(0,0,0,w,22,hacka.titlebar) ничья.SimpleText("Info","]]..b.menufont..[[",5,2.5,hacka.titletext) поверхность.Поверхность SetDrawColor(hacka.border).DrawOutlinedRect(0,0,w,h) draw.SimpleText("Health:" .. ply:Health (),"]]..b.menufont..[[",2.5,25,hacka.colortext) ничья.SimpleText("Armor:" .. ply:Armor(),"]]..b.menufont..[[",2.5,25 + 15,hacka.colortext)  рисовать.SimpleText("Скорость:" .. математика.Круглый(ply:GetVelocity():Length()),"]]..b.menufont..[[",2.5,25 + 15 + 15,hacka.colortext) ничья.SimpleText("Server:" .. GetHostName(),"]]..b.menufont..[[",2.5,70,hacka.colortext) ничья.SimpleText("Online:" .. #player.GetAll(),"]]..b.menufont..[[",2.5,85,hacka.colortext) ничья.SimpleText("Карта:" .. игра.GetMap(),"]]..b.menufont..[[",2.5,100,hacka.colortext) ничья.SimpleText("Entities:" .. math.Круглый(энц.getCount() - игрок.getCount() * 12),"]]..б.менуфонт..[[",2.5,115,hacka.colortext)  рисовать.SimpleText("Frames:" .. math.Round(1 / FrameTime()),"]]..b.menufont..[[",2.5,130,hacka.colortext) ничья.SimpleText("Ping:" .. ply:Ping(),"]]..b.menufont..[[",2.5,145,hacka.colortext) ничья.SimpleText("Date:" .. os.date("%d %b %Y"),"]]..b.menufont..[[",2.5,160,hacka.colortext) ничья.SimpleText("Time:" .. os.date("%H:%M:%S"),"]]..b.menufont..[[",2.5,175,hacka.colortext) ничья.SimpleText("Tickrate:" .. math.Раунд(1 / двигатель.tickInterval()),"]]..b.menufont..[[",2.5,190,hacka.colortext)  ​рисовать.SimpleText("Gamemode:" .. движок.ActiveGamemode(),"]]..b.menufont..[[",2.5,190 + 15,hacka.colortext) если aimtarget и em.isValid(aimtarget) и aimtarget:IsPlayer() затем рисуют.SimpleText("Target Aim: "..aimtarget:Name(),"]]..b.menufont..[[",2.5,190 + 15 + 15,hacka.colortext) еще ничья.SimpleText("Target Aim: None","]]..b.menufont..[[",2.5,190 + 15 + 15,hacka.colortext) end local weaons=ply:GetActiveWeapon() если isValid(weaons) то  рисовать.SimpleText("Clip:" ..weaons:Clip1() .."/" .. ply:GetAmmoCount(weaons:GetPrimaryAmmoType()),"]]..b.menufont..[[",2.5,190 + 15 + 15 + 15,hacka.colortext) end end end if isValid(hacka.info) и не гей["WH_INFO"], не паникей и не ИсВалид(hacka.info) затем hacka.info:Remove() end if gay["OBSBYPASS"] and not isValid(hacka.obs) then hacka.obs = vgui.Create("DFrame") hacka.obs:setSize(ScrW(),ScrH()) hacka.obs:SetPos(0,0) hacka.obs:setTitle("") hacka.obs:SetDraggable(false) hacka.obs:ShowCloseButton(false) hacka.obs.Paint=hacka.esp end если isValid(hacka.obs) и не gay["OBSBYPASS"] или panickey и isValid(hacka.obs), то hacka.obs:Remove() end end timercheck() timer.Create("]]..b.таймер..[[",2,0,timercheck) end) local a=GetRenderTarget("AntiSG"..os.time(),ScrW(),ScrH()) hook.Add("ShutDown","]]..b.hooks["ShutDown"]..[[",function() if gay["MISC_SPECTATORLIST"]and isValid(hacka.specb) then local pos,otsos=hacka.specb:getPos() if isValid(hacka.specb)then zapic("MISC_SPECTATORLIST_X",pos) zapic("MISC_SPECTATORLIST_Y",otsos) end end if gay["WH_MIRROR"]и isValid(hacka.mirror), затем local pos, otsos=hacka.mirror:getPos() zapic("WH_MIRROR_X",pos) zapic("WH_MIRROR_Y",otsos) end if gay["WH_RADAR"]and isValid(hacka.ac) затем local pos,otsos=hacka.ac:getPos() zapic("WH_RADAR_X",pos) zapic("WH_RADAR_Y",otsos) end if gay["WH_INFO"]and isValid(hacka.info) затем local pos,otsos=hacka.info:getPos() zapic("WH_INFO_X",pos) zapic("WH_INFO_Y",otsos) end if gay["MISC_ADMINLIST"]and isValid(hacka.adminlist) then local pos,otsos=hacka.adminlist:getPos() zapic("MISC_ADMINLIST_X",pos) zapic("MISC_ADMINLIST_Y",otsos) end if not gay["OBSBYPASS"]затем возвращает конец рендеринга.SetRenderTarget() end) локальная функция buttoncheckbind(a) local ply=LocalPlayer() если a==0, то возвращает end, если pm.IsTyping(ply) затем возвращает false end, если ввод.IsKeyDown(a или 87) или input.isMouseDown(a или 87) затем возвращает true end возвращает false end local function box2d(b) local c, d = b:OBBMins(), b:OBBMaxs() local e = { Вектор(c.x, c.y, c.z), Вектор(c.x, c.y, d.z), Вектор(c.x, d.y, c.z), Вектор(c.x, d.y, c.z), Вектор(d.x, c.y, d.z), Вектор(d.x, d.y, c.z), Вектор(d.x, d.y, d.z)  }  local f, g, h, i = ScrW() * 2, ScrH() * 2, 0, 0 for j, k in next, e do local l = b:localToWorld(k):ToScreen() f, g = math.min(f, l.x), math.min(g, l.y) h, i = math.max(h, l.x), math.max(i, l.y) end return f, g, h, i end function hacka.onitify(a) if panickey затем возвращает конец, если isValid(hacka.rt)или hacka.rt, то hacka.rt:Hide()hacka.rt=nil end surface.setFont("]]..b.font22 ..[[") local b=select(1,surface.getTextSize(a)+10 hacka.rt=vgui.Create("DPanel") hacka.rt:setSize(b,38-6) hacka.rt:SetPos(-b,10) hacka.rt.Paint=функция(self,c,d) рисовать.RoundedBox(0,0,0,c,d,hacka.windowbg) ничья.SimpleText(a,"]]..b.font22 ..[[",5,3,hacka.colortext) поверхность.Поверхность SetDrawColor(hacka.border).DrawOutlinedRect(0,0,c,d) конец hacka.rt:moveTo(10,10,.2,0,-1,функция() таймер.Простой(gay["NOIFICATION_SPEED"],функция() hacka.rt:moveTo(-b,10,.2,0,-1) end) end) end hook.Add("PreDrawViewModel","]]..b.hooks["PreDrawViewModel"]..[[",function(vm) if panickey then return end if panic then return end if gay["WH_VMCHAMS"] then if not vm then return end render.SetLightingMode(2) для k, v в next, vm:GetMaterials() do if v:find("v_hands") then оказывать.MaterialOverrideByIndex(k - 1, chamsweaponmaterial2) рендеринг.SetColorModulation(gay["WH_VMCHAMS_COLOR"]["r"] / 255,gay["WH_VMCHAMS_COLOR"]["g"] / 255,gay["WH_VMCHAMS_COLOR"]["b"] / 255) else render.MaterialOverrideByIndex(k - 1, chamsweaponmaterial).SetColorModulation(gay["WH_VMCHAMS_COLOR"]["r"] / 255,gay["WH_VMCHAMS_COLOR"]["g"] / 255,gay["WH_VMCHAMS_COLOR"]["b"] / 255) end end end end) крюк.Add("PostDrawViewModel","]]..b.hooks["PostDrawViewModel"]..[[",function(vm) if panickey then return end if panic then return end if gay["WH_FULLBRIGHT"] then render.SetLightingMode(0)  end if gay["WH_VMCHAMS"] then if not vm then return end render.SetLightingMode(0) для k, v в next, vm:GetMaterials() do render.MaterialOverrideByIndex(k - 1, nil) end end end) local function checknamesinfuck(p) local b="" if p.Getamount then b=b.."("..p:Getamount()..")"end --if p.GetInk then b=b.."("..p:GetInk()..")"end if p.GetHP then b=b.."("..p:GetHP()..")"end --if p.GetLastPrint then b=b.."("..p:GetLastPrint()..")"end if p.Getowning_ent then if isValid(p:Getowning_ent())then b=b.."("..p:Getlastprint ()..")"end end --if p.Getcount then b=b.."("..p:Getcount()..")"end --if p.Getcontents then b=b.."("..p:Getcontents()..")"end --if p.printName then b=b.."("..p.printName..")"end if p.GetMoney then b=b.."($"..p:GetMoney()..")"end --if p.GetPTemp then b=b.."("..p:GetPTemp().."В°C)"end --if p.GetUpgradelvl then b=b.."("..p:GetUpgradelvl()..")"end --if p.GetActivator then b=b.."("..p:GetActivator()..")"end if p.PrintedMoney then b=b.."("..p.PrintedMoney..")"end --if p.Battery then b=b.."("..p.Battery.."%)"end return b конечная локальная функция CornerBox(x, y, w, h) local bWidth = w local bHeight = h  ​Поверхность.Нарисуйте линию(x, y, x, y + (bHeight / 5)) поверхности.Нарисуйте линию(x, y, x + (bWidth / 5), y) поверхности.Нарисуйте линию(x + bWidth, y, x + bWidth - (bWidth / 5), y) поверхности.Нарисуйте линию(x + bWidth, y, x + bWidth, y + (bHeight / 5)) поверхности.Нарисуйте линию(x, y + bHeight, x + (bWidth / 5), y + bHeight) поверхности.Нарисуйте линию(x, y + bHeight, x, y + bHeight - (bHeight / 5)) поверхности.Нарисуйте линию(x + bWidth, y + bHeight, x + bWidth - (bWidth / 5), y + bHeight) поверхности.DrawLine(x + bWidth, y + bHeight, x + bWidth, y + bHeight - (bHeight / 5)) end function hacka.esp() if panickey then return end если паника затем возвращает end local ply=LocalPlayer() xpcall(function ()), если gay["AIM_DRAWFOV"] и gay["AIM_ENABLED"], то поверхность.drawCircle(ScrW() / 2, ScrH() / 2, gay["AIM_FOV"] * 4, gay["WH_FOV_COLOR"]) end if gay["WH_CURSORENTITYNAME"]then local ent=ply:GetEyeTrace().Entity:getClass() local ents=ply:GetEyeTrace().Entity if ent!="worldspawn"then local tw, th = surface.getTextSize(ent) draw.SimpleText(ent, "ChatFont", ents:localToWorld(ents:OBBCenter()):ToScreen().x - tw / 2,ents:localToWorld(ents:OBBCenter()):ToScreen().y - th / 2 , gay["WH_CURSORENTITYNAME_COLOR"] ) end end если гей["WH_SNAPLINE"], то если aimtarget и em.isValid(aimtarget) затем local pos = vm.ToScreen(em.localToWorld(aimtarget, em.OBBCenter(aimtarget)) поверхность.SetDrawColor(gay["WH_SNAPLINE_COLOR"]) поверхность.DrawLine(ScrW() / 2, ScrH() / 2, pos.x, pos.y) end end if gay["CROSSHAIR_ENABLED"]then local A=ScrH()/2 local z=ScrW()/2 local Z=gay["CROSSHAIR_SIZE"] if gay["CROSSHAIR_FOV"]then if Z>4 then draw.RoundedBox(99,z-Z/2,A-Z/2,Z,Z,gay["CROSSHAIR_COLOR"]) end end if gay["CROSSHAIR_CROSSHAIR"]then surface.SetDrawColor(gay["CROSSHAIR_COLOR"])  Поверхность.Нарисуйте линию(z+Z,A,z-Z,A) поверхности.DrawLine(z,A+Z,z,A-Z) end if gay["CROSSHAIR_BOX"]then surface.SetDrawColor(gay["CROSSHAIR_COLOR"]) поверхность.DrawOutlinedRect(z-Z-2,A-Z-2,(Z+2)*2+1,(Z+2)*2+1) end end if gay["MISC_KEYPADLOGGER"] then for b, q in next, ents.FindByClass("Keypad") do if q:GetStatus() == 1 then if gay["KEYPADTYPE"] == 1 then local kl = q:GetText() if kl=="***" or kl=="**" or kl=="**" or kl=="* * " then continue end hacka.keypadlogger[q] = kl else local kl = q:GetDisplayText() если kl=="****" или kl=="***" или kl=="**" или kl=="*", то продолжить end hacka.keypadlogger[q] = kl end end if hacka.keypadlogger[q], то если q:getPos():Расстояние(ply:getPos()) setFont(hacka.fontentitysg) локальный tw, th = поверхность.getTextSize(hacka.keypadlogger[q]) поверхность.SetTextColor(gay["MISC_KEYPADLOGGER_COLOR"]); surface.SetTextPos(q:LocalToWorld(q:OBBCenter()):ToScreen().x-tw/2,q:LocalToWorld(q:OBBCenter()):ToScreen().y-th/2); поверхность.DrawText(hacka.keypadlogger[q]); end end end для k, v в next, player.GetAll() do if v~=ply and vm.Distance( em.getPos(v), em.getPos(ply) ) y - pos2.y local w = h / 2.2 local esp_up = 7 local esp_down = 7 local esp_right = 0 local esp_left = 0 local isfriend=gay["СПИСОК ДРУЗЕЙ"][v:SteamID()]  Поверхность.setFont(hacka.fontplayersg) if gay["WH_HEALTH"] then local function healthbar() local hp = v:Health() * h / 100 if hp > h then hp = h end local diff = h - hp if gay["WH_HEALTH_POSITION"] == 1 then surface.SetDrawColor(0, 0, 0, 255); поверхность.drawRect(поз.x - w / 2 + 1, поз.y - h - 1, w + 2, 3); поверхность.SetDrawColor ((100 - em.Health(v) ) * 2.55, em.Health(v) * 2.55, 0, 255); поверхность.drawRect(поз.x - w / 2 + 2, поз.y - h , л. с./2.2, 1);  elseif gay["WH_HEALTH_POSITION"] = = 2 затем поверхность.SetDrawColor(0, 0, 0, 255); поверхность.drawRect(pos.x - w / 2 + 1, pos.y, w + 2, 3); поверхность.SetDrawColor ((100 - em.Health(v) ) * 2.55, em.Health(v) * 2.55, 0, 255); поверхность.drawRect(pos.x - w / 2 + 2, pos.y +1 , hp/2.2, 1); elseif gay["WH_HEALTH_POSITION"] == 3 then surface.SetDrawColor(0, 0, 0) поверхность.drawRect(поз.x + w / 2 - 5 + 5, поз.y - h - 1, 3, h + 2) поверхность.SetDrawColor((100 - v:Health()) * 2.55, v:Health() * 2.55, 0, 255)  Поверхность.drawRect(поз.x + w / 2 - 4 + 5, поз.y - h + diff, 1, hp) elseif gay["WH_HEALTH_POSITION"] == 4 затем поверхность.SetDrawColor(0, 0, 0) поверхность.drawRect(поз.x - w / 2 - 5, поз.y - h - 1, 3, h + 2) поверхность.SetDrawColor ((100 - v:Health()) * 2.55, v:Health() * 2.55, 0, 255) поверхность.drawRect(поз.x - w / 2 - 4, поз.y - h + diff, 1, hp) end end local function showhealth() local text = v:Health() local color=Color ((100 - v:Health()) * 2.55, v:Health() * 2.55, 0) локальная tw, th = поверхность.getTextSize(text) если gay["WH_HEALTH_POSITION"] = = 1, то surface.SetTextPos(pos.x-tw/2,pos.y-h+2-th-esp_up); esp_up = esp_up + th elseif gay["WH_HEALTH_POSITION"] == 2 then surface.SetTextPos(поз.x-tw/2,поз.y-th/2+5+esp_down); esp_down = esp_down + th elseif gay["WH_HEALTH_POSITION"] = = 3 затем поверхность.SetTextPos(pos.x+w/2+10,pos.y-h+esp_right); esp_right = esp_right + th elseif gay["WH_HEALTH_POSITION"] == 4 then surface.SetTextPos(pos.x-w/2-tw-10,pos.y-h+esp_left); esp_left = esp_left + th торцевая поверхность.SetTextColor(цвет); поверхность.DrawText(текст); end  if gay["WH_HEALTH_TYPETYPE"] == "Text + Bar" then showhealth(v) healthbar(v) elseif gay["WH_HEALTH_TYPETYPE"] == "Text" then showhealth(v) elseif gay["WH_HEALTH_TYPETYPE"] == "Bar" then healthbar(v) end end if gay["WH_ARMOR"] then if gay["WH_ARMOR_TYPETYPE"] == "Text + Bar" then local text = v:Armor() local tw, th = surface.getTextSize(text) if gay["WH_ARMOR_POSITION"] == 1 then surface.SetTextPos(pos.x-tw/2,pos.y-h+2-th-esp_up); esp_up = esp_up + th elseif gay["WH_ARMOR_POSITION"] == 2 затем поверхность.SetTextPos(поз.x-tw/2,поз.y-th/2+5+esp_down); esp_down = esp_down + th elseif gay["WH_ARMOR_POSITION"] == 3 then surface.SetTextPos(pos.x+w/2+10,pos.y-h+esp_right); esp_right = esp_right + th elseif gay["WH_ARMOR_POSITION"] == 4 then surface.SetTextPos(pos.x-w/2-tw-10,pos.y-h+esp_left); esp_left = esp_left + th торцевая поверхность.SetTextColor(0,0,255); поверхность.DrawText(текст); если gay["WH_ARMOR_POSITION"] = = 4, то локальный hp = v:Armor() * h / 100, если hp > h, то  hp = h end local diff = h - hp surface.SetDrawColor(Color(0,0,255)) поверхность.drawRect(поз.x - w / 2, поз.y - h + diff, 1, hp) else local hp = v:Armor() * h / 100 if hp > h then hp = h end local diff = h - hp surface.SetDrawColor(Color(0,0,255))  Поверхность.drawRect(pos.x + w / 2 + 10, pos.y - h + diff, 1, hp) end elseif gay["WH_ARMOR_TYPETYPE"] == "Text" then local text = v:Armor() local tw, th = surface.getTextSize(text) if gay["WH_ARMOR_POSITION"] == 1 then surface.SetTextPos(pos.x-tw/2,pos.y-h+2-th-esp_up); esp_up = esp_up + th elseif gay["WH_ARMOR_POSITION"] == 2 затем поверхность.SetTextPos(pos.x-tw/2,pos.y-th/2+5+esp_down); esp_down = esp_down + th elseif gay["WH_ARMOR_POSITION"] == 3 затем поверхность.SetTextPos(pos.x+w/2+10,pos.y-h+esp_right); esp_right = esp_right + th elseif gay["WH_ARMOR_POSITION"] = = 4 затем поверхность.SetTextPos(pos.x-w/2-tw-10,pos.y-h+esp_left); esp_left = esp_left + th торцевая поверхность.SetTextColor(Color(0,0,255)); поверхность.DrawText(text); else if gay["WH_ARMOR_POSITION"] == 4 then local hp = v:Armor() * h / 100 if hp > h then hp = h end local diff = h - hp surface.SetDrawColor(0, 0, 0, 255)  ​Поверхность.drawRect(поз.x - w / 2 - 1, поз.y - h - 1, 3, h + 2) поверхность.SetDrawColor(Color(0,0,255)) поверхность.drawRect(поз.x - w / 2, поз.y - h + diff, 1, hp) else local hp = v:Armor() * h / 100 if hp > h then hp = h end local diff = h - hp surface.SetDrawColor(0, 0, 0, 255)  Поверхность.drawRect(поз.x + w / 2 - 5 + 10, поз.y - h - 1, 3, h + 2) поверхность.SetDrawColor(Цвет(0,0,255)) поверхность.drawRect(поз.x + w / 2 - 4 + 10, поз.y - h + diff, 1, hp) end end end if gay["WH_JOB"] then local text = team.getName(v:Team()) local tw, th = surface.getTextSize(text) if gay["WH_JOB_POSITION"] == 1 then surface.SetTextPos(pos.x-tw/2,pos.y-h+2-th-esp_up); esp_up = esp_up + th elseif gay["WH_JOB_POSITION"] == 2 тогда Поверхность.SetTextPos(pos.x-tw/2,pos.y-th/2+5+esp_down); esp_down = esp_down + th elseif gay["WH_JOB_POSITION"] == 3 then surface.SetTextPos(pos.x+w/2+10,pos.y-h+esp_right); esp_right = esp_right + th elseif gay["WH_JOB_POSITION"] = = 4 затем поверхность.SetTextPos(pos.x-w/2-tw-10,pos.y-h+esp_left); esp_left = esp_left + th торцевая поверхность.SetTextColor(команда.getColor(v:Team ())); поверхность.DrawText(текст); end if gay["WH_MONEY"] and DarkRP then local text = DarkRP.formatMoney(v:getDarkRPVar("money")) локальная tw, th = поверхность.getTextSize(text) если gay["WH_MONEY_POSITION"] = = 1 то Поверхность.SetTextPos(pos.x-tw/2,pos.y-h+2-th-esp_up); esp_up = esp_up + th elseif gay["WH_MONEY_POSITION"] == 2 затем поверхность.SetTextPos(pos.x-tw/2,pos.y-th/2+5+esp_down); esp_down = esp_down + th elseif gay["WH_MONEY_POSITION"] == 3 затем поверхность.SetTextPos(pos.x+w/2+10,pos.y-h+esp_right); esp_right = esp_right + th elseif gay["WH_MONEY_POSITION"] == 4 затем поверхность.SetTextPos(pos.x-w/2-tw-10,pos.y-h+esp_left); esp_left = esp_left + th торцевая поверхность.SetTextColor(gay["WH_MONEY_COLOR"]); surface.DrawText(текст); end if gay["WH_WEAPON"] then local weaponnn = v:GetActiveWeapon(); если gay["WH_WEAPON_TYPETYPE"] == "All", то для _, v в next, v:GetWeapons() сделайте local tw, th = surface.getTextSize(v:GetPrintName()) если gay["WH_WEAPON_POSITION"] == 1, то surface.SetTextPos(pos.x-tw/2,pos.y-h+2-th-esp_up); esp_up = esp_up + th elseif gay["WH_WEAPON_POSITION"] == 2 затем поверхность.SetTextPos(pos.x-tw/2,pos.y-th/2+5+esp_down); esp_down = esp_down + th elseif gay["WH_WEAPON_POSITION"] == 3 then surface.SetTextPos(pos.x+w/2+10,pos.y-h+esp_right); esp_right = esp_right + th elseif gay["WH_WEAPON_POSITION"] = = 4 затем поверхность.SetTextPos(pos.x-w/2-tw-10,pos.y-h+esp_left); esp_left = esp_left + th торцевая поверхность.SetTextColor(gay["WH_WEAPON_COLOR"]); поверхность.DrawText(v:GetPrintName()); end elseif weaponnn и isValid(weaponnn)затем local clipweaponf=weaponnn:Clip1() local weaponnames = (gay["WH_WEAPON_SHOWCLIP"]и clipweaponf~=-1)и weaponnn:GetPrintName().."("..clipweaponf.."/" .. v:GetAmmoCount(weaponnn:GetPrimaryAmmoType ()..")" или weaponnn:GetPrintName () local tw, th = surface.getTextSize(имена оружия) если gay["WH_WEAPON_POSITION"] == 1, то поверхность.SetTextPos(pos.x-tw/2,pos.y-h+2-th-esp_up); esp_up = esp_up + th elseif gay["WH_WEAPON_POSITION"] == 2 затем поверхность.SetTextPos(pos.x-tw/2,pos.y-th/2+5+esp_down); esp_down = esp_down + th elseif gay["WH_WEAPON_POSITION"] == 3 затем поверхность.SetTextPos(pos.x+w/2+10,pos.y-h+esp_right); esp_right = esp_right + th elseif gay["WH_WEAPON_POSITION"] == 4 затем поверхность.SetTextPos(pos.x-w/2-tw-10,pos.y-h+esp_left); esp_left = esp_left + th торцевая поверхность.SetTextColor(gay["WH_WEAPON_COLOR"]); surface.DrawText(weaponnames); end end  если gay["WH_USERGROUP"] , то local tw, th = surface.getTextSize(pm.GetUserGroup(v)) если gay["WH_USERGROUP_POSITION"] == 1, то surface.SetTextPos(pos.x-tw/2,pos.y-h+2-th-esp_up); esp_up = esp_up + th elseif gay["WH_USERGROUP_POSITION"] == 2 затем поверхность.SetTextPos(pos.x-tw/2,pos.y-th/2+5+esp_down); esp_down = esp_down + th elseif gay["WH_USERGROUP_POSITION"] == 3 затем поверхность.SetTextPos(pos.x+w/2+10,pos.y-h+esp_right); esp_right = esp_right + th  elseif gay["WH_USERGROUP_POSITION"] == 4 затем поверхность.SetTextPos(pos.x-w/2-tw-10,pos.y-h+esp_left); esp_left = esp_left + th торцевая поверхность.SetTextColor(gay["WH_USERGROUP_COLOR"]); surface.DrawText(pm.GetUserGroup(v)); end if gay["WH_NAME"] then local tw, th = surface.getTextSize(pm.Name(v)) если gay["WH_NAME_POSITION"] == 1, то поверхность.SetTextPos(pos.x-tw/2,pos.y-h+2-th-esp_up); esp_up = esp_up + th elseif gay["WH_NAME_POSITION"] == 2 then Поверхность.SetTextPos(pos.x-tw/2,pos.y-th/2+5+esp_down); esp_down = esp_down + th elseif gay["WH_NAME_POSITION"] == 3 затем поверхность.SetTextPos(pos.x+w/2+10,pos.y-h+esp_right); esp_right = esp_right + th elseif gay["WH_NAME_POSITION"] == 4 затем поверхность.SetTextPos(pos.x-w/2-tw-10,pos.y-h+esp_left); esp_left = esp_left + th торцевая поверхность.SetTextColor ((gay["WH_NAME_FRIENDCOLOR"] и isfriend)и gay["WH_FRIENDCOLOR"] или gay["WH_NAME_COLOR"]); surface.DrawText(pm.Name(v)); конец  если gay["WH_SKELETON"] , то local origin = em.getPos(v); для i = 1, em.GetBoneCount(v) do local parent = em.GetBoneParent(v, i); если(!родитель), то продолжить; end local bonepos, parentpos = em.GetBonePosition(v, i), em.GetBonePosition(v, родитель); if(!bonepos || !parentpos || bonepos == origin) затем продолжить; end local bs, ps = vm.ToScreen(bonepos), vm.ToScreen(parentpos); если gay["WH_SKELETON_COLORTYPE"] == "Team", то surface.SetDrawColor(команда.getColor(v:Team ())) другая поверхность.SetDrawColor(gay["WH_SKELETON_COLOR"]) торцевая поверхность.DrawLine(bs.x, bs.y, ps.x, ps.y); end end  if gay["WH_HITBOX"]then for i = 0, em.GetHitBoxGroupCount(v) - 1 do local count = em.GetHitBoxCount(v, i); if(!count) then continue; end for hitbox = 0, count do local bone = em.GetHitBoxBone( v, hitbox, i ); if(!bone) then continue; end local min, max = em.gethitbounds( v, hitbox, i ); local bonepos, boneang = em.GetBonePosition( v, bone ); cam.Start3D(); if gay["WH_HITBOX_COLORTYPE"] == "Team" then render.DrawWireframeBox(bonepos,boneang,min,max,team.getColor(v:Team()),true) else render.DrawWireframeBox(bonepos,boneang,min,max,gay["WH_HITBOX_COLOR"],true) end cam.End3D(); end end если gay["WH_CHAMS"] и not gay["OBSBYPASS"] , то local colorFix = (1 / 255) local chamColVis = (gay["WH_CHAMS"]=="Team")и team.getColor(v:Team()) или gay["WH_CHAMS_COLOR"] если gay["WH_CHAMS_TYPE"] == "XYZ" , то cam.Start3D(EyePos(), EyeAngles ()) cam.IgnoreZ(true) рендеринг.SuppressEngineLighting(true) em.DrawModel(v); cam.IgnoreZ(false) рендеринг.SuppressEngineLighting(false) cam.End3D()  elseif gay["WH_CHAMS_TYPE"] == "Textured", затем cam.Start3D( EyePos(), EyeAngles() ) рендеринг.SetColorModulation( chamColVis.r * colorFix, chamColVis.g * colorFix, chamColVis.b * colorFix ) рендеринг.SetBlend( chamColVis.a * colorFix) рендеринг.MaterialOverride( texturedCham ) em.SetRenderMode(v, 1); em.DrawModel(v); cam.End3D() elseif gay["WH_CHAMS_TYPE"] == "Flat", затем cam.Start3D( EyePos(), EyeAngles() ) render.SuppressEngineLighting( true ) рендеринг.SetColorModulation( chamColVis.r * colorFix, chamColVis.g * colorFix, chamColVis.b * colorFix ) оказывать.SetBlend( chamColVis.a * colorFix) рендеринг.MaterialOverride( texturedCham ) em.SetRenderMode(v, 1); em.DrawModel(v); render.SuppressEngineLighting( false ) cam.End3D() elseif gay["WH_CHAMS_TYPE"] == "Wireframe", затем cam.Start3D( EyePos(), EyeAngles() ) render.SetColorModulation( chamColVis.r * colorFix, chamColVis.g * colorFix, chamColVis.b * colorFix ) рендеринг.SetBlend( chamColVis.a * colorFix) рендеринг.MaterialOverride( wireframeCham ) em.SetRenderMode(v, 1); em.DrawModel(v); cam.End3D() end end if gay["WH_BOX"] then  если gay["WH_BOX_TYPETYPE"] == "2d" или gay["WH_BOX_TYPETYPE"] == "2d fat", то local pos = em.getPos(v) local pos, aj = vm.ToScreen(pos - Вектор(0, 0, 5)), vm.ToScreen(pos + Вектор(0, 0, 70)) local u = pos.y - aj.y local t = u / 2.2 если gay["WH_BOX_FRIENDCOLOR"] и isfriend, то поверхность.SetDrawColor(gay["WH_FRIENDCOLOR"]) elseif gay["WH_BOX_TYPECOLOR"] == "Custom" затем surface.SetDrawColor(gay["WH_BOX_COLOR"])  ​еще одна поверхность.SetDrawColor(команда.getColor(v:Team ())) end if gay["WH_BOX_TYPETYPE"] == "2d fat" then surface.DrawOutlinedRect(поз.x - t / 2, поз.y - u, t, u,2) другая поверхность.DrawOutlinedRect(поз.x - t / 2, поз.y - u, t, u,1) end elseif gay["WH_BOX_TYPETYPE"] == "Corner" or gay["WH_BOX_TYPETYPE"] == "Corner fat" then if gay["WH_BOX_FRIENDCOLOR"] and isfriend then  Поверхность.SetDrawColor(gay["WH_FRIENDCOLOR"]) elseif gay["WH_BOX_TYPECOLOR"] == "Custom" затем surface.SetDrawColor(gay["WH_BOX_COLOR"]) else surface.SetDrawColor(команда.getColor(v:Team ())) end CornerBox(pos.x - w / 2, pos.y -h, w, h) если gay["WH_BOX_TYPETYPE"] == "Corner fat", то CornerBox(pos.x - w / 2 + 1, pos.y - h + 1, w - 2, h - 2); end else cam.Start3D() if gay["WH_BOX_FRIENDCOLOR"] and isfriend then  оказывать.DrawWireframeBox(v:getPos(),Angle(0, v:EyeAngles().y, 0),v:OBBMins(),v:OBBMaxs(),gay["WH_FRIENDCOLOR"]) elseif gay["WH_BOX_TYPECOLOR"] == "Custom" затем рендеринг.DrawWireframeBox(v:getPos(),Angle(0, v:EyeAngles().y, 0),v:OBBMins(),v:OBBMaxs(),gay["WH_BOX_COLOR"]) else render.DrawWireframeBox(v:getPos(),Angle(0, v:EyeAngles().y, 0),v:OBBMins(),v:OBBMaxs(),team.getColor(v:Team ())) end cam.End3D() end end end  ​end end для _, p в next, ents.FindByClass("*") do local nameent = p:getClass() if gay["WH_ENTITY_DORMANT"] and p:IsDormant()then continue end if gay["ENTITYLIST"][nameent] then if p:getPos():Distance(ply:getPos()) IgnoreZ(правда)  оказывать.SuppressEngineLighting(true) em.DrawModel(p); cam.IgnoreZ(ложный) рендер.SuppressEngineLighting(false) cam.End3D() elseif gay["WH_ENTITY_CHAMS_TYPE"] == "Textured" then cam.Start3D( EyePos(), EyeAngles() ) render.SetColorModulation( chamColVis.r * colorFix, chamColVis.g * colorFix, chamColVis.b * colorFix ) рендеринг.SetBlend( chamColVis.a * colorFix) рендеринг.MaterialOverride( texturedCham ) em.SetRenderMode(p, 1); em.DrawModel(p); cam.End3D()  elseif gay["WH_ENTITY_CHAMS_TYPE"] == "Flat", затем cam.Start3D( EyePos(), EyeAngles() ) рендеринг.SuppressEngineLighting( true ) рендеринг.SetColorModulation( chamColVis.r * colorFix, chamColVis.g * colorFix, chamColVis.b * colorFix ) рендеринг.SetBlend( chamColVis.a * colorFix) рендеринг.MaterialOverride( texturedCham ) em.SetRenderMode(p, 1); em.DrawModel(p); render.SuppressEngineLighting( false ) cam.End3D() elseif gay["WH_ENTITY_CHAMS_TYPE"] == "Wireframe" then cam.Start3D( EyePos(), EyeAngles() ) оказывать.SetColorModulation( chamColVis.r * colorFix, chamColVis.g * colorFix, chamColVis.b * colorFix ) рендеринг.SetBlend( chamColVis.a * colorFix) рендеринг.MaterialOverride( wireframeCham ) em.SetRenderMode(p, 1); em.DrawModel(p); cam.End3D() end end if gay["WH_ENTITY_NAME"] then local shitf=(gay["WH_ENTITY_NAME_DETAILS"])and nameent.." " ..checknamesinfuck(p) or nameent surface.setFont(hacka.fontentitysg) локальный tw, th = поверхность.getTextSize(shitf) если gay["WH_ENTITY_NAME_COLORTYPE"]=="Custom"то Поверхность.SetTextColor(gay["WH_ENTITY_NAME_COLOR"]); else surface.SetTextColor(gay["ENTITYLISTclr"][nameent.."_Color"] or Color(255,255,255)); торцевая поверхность.SetTextPos(p:LocalToWorld(p:OBBCenter()):ToScreen().x-tw/2,p:LocalToWorld(p:OBBCenter()):ToScreen().y-th/2); поверхность.DrawText(shitf); end if gay["WH_ENTITY_BOX"] then if gay["WH_ENTITY_BOX_TYPE"] == "3d" then local pos = em.getPos(p); local ang = em.GetAngles(p); local min, max = em.GetRotatedAABB(p, em.GetCollisionBounds(p)); cam.Start3D(); if gay["WH_ENTITY_BOX_COLORTYPE"]=="Custom"then оказывать.DrawWireframeBox( pos, Angle(), min, max, gay["WH_ENTITY_BOX_COLOR"], false ); else render.DrawWireframeBox( pos, Angle(), min, max, gay["ENTITYLISTclr"] [nameent.."_Color"], false ); end cam.End3D(); иначе, если gay["WH_ENTITY_BOX_COLORTYPE"]=="Entity"и gay["ENTITYLISTclr"] [nameent.."_Color"], то поверхность.SetDrawColor(gay["ENTITYLISTclr"][nameent.."_Color"]) else surface.SetDrawColor(gay["WH_ENTITY_BOX_COLOR"]) end local x1, y1, x2, y2 = box2d(p) surface.DrawLine(x1, y1, math.min(x1 + 500, x2), y1)  ​Поверхность.DrawLine(x1, y1, x1, math.min(y1 + 500, y2)) поверхность.DrawLine(x2, y1, math.max(x2 - 500, x1), y1) поверхность.DrawLine(x2, y1, x2, math.min(y1 + 500, y2)) поверхность.Рисование(x1, y2, мат.мин(x1 + 500, x2), y2) поверхности.DrawLine(x1, y2, x1, math.max(y2 - 500, y1)) поверхность.DrawLine(x2, y2, math.max(x2 - 500, x1), y2) поверхность.DrawLine(x2, y2, x2, math.max(y2 - 500, y1)) end end end end end,функция(a)print(a)end)  конец крючка.Add("HUDPaint","]]..b.hooks["HUDPaint"]..[[",function() if gay["OBSBYPASS"]then return end hacka.esp() end) hook.Add("player_changename","]]..b.hooks["player_changename"]..[[",function(data) zapiclogs("namechange",data.oldname.." changed to "..data.newname) end) hook.Add("player_connect","]]..b.hooks["player_connect"]..[[",function(data) zapiclogs("connect",data.name.." "..data.networkid) end) hook.Add("player_disconnect","]]..b.hooks["player_disconnect"]..[[",function(data) zapiclogs("disconnect",data.name.." "..data.networkid..". "..data.reason) end) крюк.Add("player_say","]]..b.hooks["player_say"]..[[",function(player) xpcall(function() zapiclogs("chat",Player(player.userid):Nick()..": "..player.text) end,function()end) end) hook.Add("player_hurt","]]..b.hooks["player_hurt"]..[[",function(b) local ply=LocalPlayer() local c="unknown" local d="map" local e="unknown weapon" local s=0 for i,j in next, player.GetAll() do if j:userId()==b.userid then c=j:Nick() s=j:Health() end if j:userId()==b.attacker then d=j:Nick() if j:GetActiveWeapon()and j:GetActiveWeapon():isValid()then e=j:GetActiveWeapon():GetPrintName() end end end если gay["HITSOUNDS"]и d==ply:Nick(), то поверхность.PlaySound("buttons/bell1.wav") end if gay["NOIFICATION_DAMAGEDMEBY"]and c==ply:Nick()then hacka.onitify("You damaged by "..d.." with "..e) end zapiclogs("damage",d.." damage "..string.Заменить(b.health-s,"-","").." hp "..c.." с "..e) концевым) крючком.Add("entity_killed","]]..b.hooks["entity_killed"]..[[",function(qwe) local ply="unknown" local atk="map" local wep="unknown weapon" local retard=game.GetWorld() local killah=игра.GetWorld() локальный причинитель=ents.GetByIndex(qwe.entindex_inflictor) for _,p in next, player.GetAll() do  if p:EntIndex()==qwe.entindex_killed then 
ply=p:Nick()
retard=p 
end
...
xpcall(function()
 ...
 ​RunConsoleCommand("say", gay["SPAM_KILLSAY"]) [math.random(1, #gay["SPAM_KILLSAY"])]) else RunConsoleCommand("say", "/ooc " .. gay["SPAM_KILLSAY"] [math.random(1, #gay["SPAM_KILLSAY"])]) end end,function()end end end) hook.Add("OnEntityCreated","]]..b.hooks["OnEntityCreated"]..[[",function(ent) timer.Простой(0.1,функция() для _,v в следующем, игрок.GetAll() do if v:GetEyeTrace().Entity==ent and ent:getModel() and not string.find(ent:getModel(),".bsp") then zapiclogs("prop",v:Nick().." spawned ".. ent:getModel().." ("..tostring(ent:getPos ())..")" end end end) end hook.Add("PostDraw2DSkyBox","]]..b.hooks["PostDraw2DSkyBox"]..[[",function() if panickey then return end if panic then return end if gay["WH_NOSKY"] then render.Clear(gay["WH_NOSKY_COLOR"] ["r"],gay["WH_NOSKY_COLOR"] ["g"],gay["WH_NOSKY_COLOR"] ["b"],0,true,true) end end) hook.Add("renderScene","]]..b.hooks["renderScene"]..[[",function() if panickey then return end if panic then return end if gay["WH_FULLBRIGHT"] then render.SetLightingMode(1 и 1 или 0) end if gay["OBSBYPASS"]then local e={x=0,y=0,w=ScrW(),h=ScrH(),dopostprocess=true,origin=b,angles=c,fov=d,drawhud=true,drawmonitors=true,drawviewmodel=true}
render.RenderView(e)
...
hook.Add("Move", "", function)
    ​if(!IsFirstTimePredicted()) then return; ed
